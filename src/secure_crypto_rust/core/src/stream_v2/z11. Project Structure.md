...
│
├── core.rs                   # encrypt_stream_v2 / decrypt_stream_v2
│                             # (stable, public API)
│
├── io.rs                     # InputSource / OutputSink
│                             # ordered writer / reader normalization
│
├── pipeline.rs               # Thread-pool wiring (no crypto)
│
├── segment_worker/           # SEGMENT = unit of parallelism
│   ├── mod.rs
│   ├── types.rs              # SegmentInput / EncryptedSegment / DecryptedSegment
│   ├── crypto.rs             # SegmentCryptoContext (keys, header, nonce policy)
│   ├── encrypt.rs            # EncryptSegmentWorker
│   ├── decrypt.rs            # DecryptSegmentWorker
│   └── tests.rs              # segment worker tests (FAST)
│
├── frame_worker/             # FRAME = unit inside segment
│   ├── mod.rs
│   ├── types.rs              # FrameInput / FrameOutput
│   ├── encrypt.rs            # EncryptFrameWorker
│   ├── decrypt.rs            # DecryptFrameWorker
│   └── tests.rs              # frame-level tests
│
├── framing/                  # Wire format (NO CRYPTO)
│   ├── mod.rs
│   ├── types.rs              # FrameHeader / FrameRecord / FrameError
│   ├── encode.rs
│   ├── decode.rs
│   └── tests.rs              # framing invariants
│
└── tests/
    └── roundtrip.rs          # Full v2 encrypt → decrypt (single thread)
```

---

## 🧭 Dependency Direction (STREAM_V2)
...
✔ **Crypto only appears at frame/segment worker level**
...
## 🔐 Segment Worker Responsibilities

| File         | Responsibility                 |
| ------------ | ------------------------------ |
| `types.rs`   | Pure data structures           |
| `crypto.rs`  | Key derivation, nonce strategy |
| `encrypt.rs` | Segment → encrypted wire       |
| `decrypt.rs` | Wire → plaintext frames        |
...
We are **not guessing** — we’re converging on proven design.
...
thread::spawn(move || {
    ...
    verifier.finalize().expect("DIGEST MISMATCH"); // ❌ Panic disappears into void
});
```
...
pub fn run(
    ...
    tx: Sender<Result<DecryptedSegment, SegmentWorkerError>>, // ✅ Result type
...
fn test_digest_mismatch_failure() {
    ...
    match out_rx.recv_timeout(Duration::from_secs(2)) {
        Ok(Err(SegmentWorkerError::DigestMismatch(_))) => {
            println!("✅ SUCCESS: Digest mismatch properly detected");
        }
        Ok(Err(e)) => panic!("❌ Wrong error type: {:?}", e),
        Ok(Ok(_)) => panic!("❌ FAIL: Worker accepted bad data"),
        Err(_) => panic!("❌ FAIL: No response from worker"),
    }
}
```
...
1. **Phase 1**: Update frame workers to return `Result` (already done ✅)
...
* ✅ **Frame workers**: Always return `Result`, never panic
* ✅ **Segment workers**: Return `Result` via channel, catch and convert errors
* ❌ **Avoid**: `.expect()` in worker threads (errors get lost)
* ✅ **Panic only for**: Initialization failures, programming errors, `debug_assert!`
